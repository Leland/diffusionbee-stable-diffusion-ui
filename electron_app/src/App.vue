...
<template>
    <div id="app">
        ...
        <div v-if="store.is_start_screen">
            <transition name="slide_show">
                <SplashScreen v-if="store.show_splash_screen" />
            </transition>
        </div>
        <ApplicationFrame ref="app_frame" v-else title="DiffusionBee - Stable Diffusion App"
            @menu_item_click_about="show_about" @menu_item_click_help="show_help" @menu_item_click_close="close_window"
            @menu_item_click_discord="menu_item_click_discord">
            <template v-slot:txt2img>
                <ImgGenerate v-if="is_mounted && stable_diffusion.is_backend_loaded"
                    :stable_diffusion="stable_diffusion" />
                <div v-else class="animatable_content_box ">
                    <LoaderModal :loading_percentage="stable_diffusion.loading_percentage"
                        :loading_desc="stable_diffusion.model_loading_msg"
                        :loading_title="stable_diffusion.model_loading_title ||'Loading model'"> </LoaderModal>
                </div>
            ...
            <template v-slot:img2img>
                ...
                <Img2Img ref="img2img" v-if="is_mounted && stable_diffusion.is_backend_loaded"
                    :stable_diffusion="stable_diffusion" />
                <div v-else class="animatable_content_box ">
                    <LoaderModal :loading_percentage="stable_diffusion.loading_percentage"
                        :loading_desc="stable_diffusion.model_loading_msg"
            ...
            <template v-slot:logs>
                <div class="animatable_content_box ">
                    ...
                    <p>
                        <span style="white-space: pre-line">{{store.logs}}</span>
                    </p>
        ...
        <LoaderModal v-if="store.global_loader_modal_msg" :loading_percentage="-1"
            :loading_title="store.global_loader_modal_msg"> </LoaderModal>
...
export default

    {
        ...
        mounted() {
            ...
            setTimeout(() => (store.is_start_screen = false), 4000);

            store.app_object = this;
            store.is_start_screen = true;
            this.is_mounted = true;

            let data = window.ipcRenderer.sendSync('load_data');
            if (data.history)
                Vue.set(store, 'history', data.history)

        },

        watch: {
            'store.is_start_screen': {
