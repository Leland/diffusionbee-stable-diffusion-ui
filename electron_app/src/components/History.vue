...
<template>
    <div class="animatable_content_box ">
        {{get_history()}}
        <div v-if="Object.values(store.history).length > 0">
            <div v-for="history_box in get_history()" :key="history_box.key" style="clear: both;">

                <div @click="delete_hist(history_box.key)" style="float:right; margin-top: 10px;" class="l_button">
                    Delete</div>
                ...
                <b-dropdown left variant="link" size="sm" toggle-class="text-decoration-none" no-caret
                    ...
                    <b-dropdown-item-button @click="share_on_arthub(history_box)">Share on ArtHub.ai
                ...
                <p class="history_box_info text_bg" style="user-select: text;">
                    ...
                    <span style="opacity: 0.5;" v-if="get_box_params_str(history_box)">
                        {{get_box_params_str(history_box)}} </span>
...
export default {
    ...
    methods: {
        delete_hist(k) {
            Vue.delete(store.history, k);
        },

        get_history() {
            let history = Object.values(store.history);
            return store.show_history_in_oldest_first ? history : history.reverse();
        },

        toggle_order() {
            return store.show_history_in_oldest_first = !store.show_history_in_oldest_first;
        },
